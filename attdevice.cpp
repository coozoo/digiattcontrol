...
void AttDevice::onSerialPortNewData(QString line) {
    ...
    if (m_inProbe) {
        if (m_probeState == ProbeWaitingSetOK && trimmed.compare("attOK", Qt::CaseInsensitive) == 0) {
            m_probeState = ProbeWaitingReadOK;
            readValue(); // logs read command
            m_probeTimer.start(1000);
            return;
        }
        if (m_probeState == ProbeWaitingReadOK && trimmed.compare("attOK", Qt::CaseInsensitive) == 0) {
        ...
        if (m_probeState == ProbeWaitingValue && match.hasMatch()) {
            ...
            if (qFuzzyCompare(val + 1, m_probeValue + 1)) { // +1 to avoid qFuzzyCompare(0,0)==false
                finishProbe(true);
                return;
            } else {
                m_probeTypeIdx++;
